...
# ===== 云端部署：安装playwright浏览器（直接输出到终端）=====
def setup_playwright():
    """确保playwright浏览器已安装"""
    try:
        # 直接输出到终端，绕过Streamlit
        import sys
        sys.__stdout__.write("📦 正在检查playwright浏览器...\n")
        ...
        if browser_path.exists():
            sys.__stdout__.write("✅ 浏览器已存在\n")
            return
        
        # 安装 Chromium
        sys.__stdout__.write("📥 正在安装 Chromium 浏览器...\n")
        subprocess.run(
            ["playwright", "install", "chromium"],
            check=True,
            timeout=300
        )
        sys.__stdout__.write("✅ playwright浏览器安装成功\n")
        
    except Exception as e:
        sys.__stdout__.write(f"⚠️ playwright安装警告: {e}\n")
...
with st.sidebar:
    ...
    show_browser = st.checkbox("👁️ 显示浏览器", value=False)
...
async def run_analysis(questions, show_browser, delay):
    ...
    try:
        ...
        status_placeholder.info("🔐 正在检查登录状态...")
        ...
        for i, question in enumerate(questions):
            ...
            status_placeholder.info(f"⏳ 正在处理 [{i+1}/{len(questions)}]: {question[:50]}...")
    ...
    for idx, result in enumerate(st.session_state.results):
        with st.expander(f"📌 问题 {idx+1}: {result['question']}", expanded=True):
            ...
            if citations:
                ...
                display_df = pd.DataFrame(citations)
